...
class RutbeSecici(discord.ui.Select):
    def __init__(self, hedef_id: int):
        ...
        super().__init__(placeholder="Rütbe seç…", min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        secilen_rutbe[self.hedef_id] = self.values[0]
        await interaction.response.send_message(f"✅ Rütbe **{self.values[0]}** seçildi. Kabul Et veya Red Et butonuna bas.", ephemeral=True)
...
class KayitView(discord.ui.View):
    ...
    async def kabul(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if rutbe_rol_id:
            ...
            hatalar.append(f"⚠️ `{rutbe_adi}` için rol ID'si ayarlanmamış.")
    ...
    async def red(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(content=f"❌ **{self.hedef.mention}** başvurusu red edildi. {dm_bilgi}", view=None)
        secilen_rutbe.pop(self.hedef.id, None)


# ══════════════════════════════════════════════
#  BAŞVURU BUTONU – HERKES BAŞVURABİLİR
# ══════════════════════════════════════════════
...
async def sasp_kayit(interaction: discord.Interaction, uye: discord.Member):
    ...
    await interaction.response.send_message(embed=embed, view=KayitView(uye))
